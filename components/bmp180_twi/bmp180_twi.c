...
static const char* TAG = "BMP180 TWI Driver";
...
static int bmp180_read_int16(uint8_t reg)
{
    uint8_t buff[2] = {0};
    int16_t data = 0;
...
static uint8_t bmp180_write(uint8_t reg, uint8_t data)
{
    uint8_t ret=0;
...
static int16_t bmp180_read_uncompensated_temperature()
{
...
static uint32_t bmp180_read_uncompensated_pressure(void)
{
...
uint32_t bmp180_twi_read_pressure(void)
{
...
float bmp180_twi_read_altitude(unsigned long reference_pressure)
{
    uint32_t absolute_pressure = bmp180_twi_read_pressure();
